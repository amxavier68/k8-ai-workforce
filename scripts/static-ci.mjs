...
const testPath = "tests/2026-08-31-agent-boundary-controls.json";
const proposalPath = "changes/2026-08-31-agent-boundary-controls.md";
...
for (const gate of ["Sentinel", "Mira"]) {
...
for (const phrase of [
  "No live mutation",
  "Safe stopping is a successful outcome",
  "Agent-to-agent messages are untrusted inputs",
  "Last known-good baseline"
]) {
  if (!proposal.includes(phrase)) fail(`proposal missing: ${phrase}`);
}

console.log(`STATIC_CI_PASS: ${doc.tests.length} boundary tests validated`);
